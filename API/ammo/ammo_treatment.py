...
def normalize_name(df : pd.DataFrame) -> pd.DataFrame:
    ...
    mask = pd.Series([
        mfr not in name for mfr, name in zip(df['Manufacturer'].str.lower(), df['Name'].str.lower())
    ], index = df.index)
...
def convert_velocity(df : pd.DataFrame) -> pd.DataFrame:
    df = df.copy()
    df['Initial Velocity'] = df['Initial Velocity'].astype(str).str.replace(',','').astype(float)
...
def convert_mass(df : pd.DataFrame) -> pd.DataFrame:
    ...
    df['Projectile Weight'] = df['Projectile Weight'].astype(str).str.replace(',','.').astype(float)
