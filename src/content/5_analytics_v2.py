...
logger.debug("Analytics Dashboard")
...
with elements("dashboard"):
    ...
    with dashboard.Grid(layout):
        with mui.Card(
            key="metrics", sx={"p": 2, "display": "flex", "flexDirection": "column"}
        ):
            ...
            mui.CardContent(
                mui.Stack(
                    ...
                    mui.Typography(f"Total Shots: {len(filtered_df)}", variant="h6"),
                    spacing=2,
                )
            )
        #
        ...
        #     )
        with mui.Paper(
            key="details", sx={"p": 2, "display": "flex", "flexDirection": "column"}
        ):
            mui.DataGrid(
                columns=[
