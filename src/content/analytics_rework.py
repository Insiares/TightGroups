...
class AnalyticsDashboard:
    ...
    def get_analytics(self) -> pd.DataFrame:
        ...
        try:
            ...
            response = requests.get(
                f"{backend_url}/scores/{st.session_state.user_id}/", headers=headers
            )
    ...
    def group_data_by_date(self, filtered_df: pd.DataFrame) -> pd.DataFrame:
        ...
        grouping_map = {"Day": "D", "Week": "W", "Month": "M"}

        grouped = (
            filtered_df.groupby(
                pd.Grouper(
                    key="created_at", freq=grouping_map[st.session_state.group_by]
                )
            )
            .agg({"group_size": ["mean", "min", "max", "count"]})
            .reset_index()
        )
    ...
    def create_sidebar_filters(self, df: pd.DataFrame):
        ...
        st.session_state.ammo_filter = st.sidebar.multiselect(
            "Select ammo", options=df["name"].unique(), default=df["name"].unique()
        )

        st.session_state.gear_filter = st.sidebar.multiselect(
            "Select gear", options=df["gear"].unique(), default=df["gear"].unique()
        )
        ...
        st.session_state.start_date, st.session_state.end_date = st.sidebar.date_input(
            "Select Date Range", value=(df["created_at"].min(), df["created_at"].max())
        )

        st.session_state.group_by = st.sidebar.selectbox(
            "Group data by", options=["Day", "Week", "Month"], index=0
        )
    ...
    def render_metrics_card(self, filtered_df: pd.DataFrame):
        ...
        return mui.Card(
            mui.CardContent(
                mui.Stack(
                    mui.Typography(
                        f"Mean Group Size: {mean_group_size:.2f}", variant="h6"
                    ),
                    mui.Typography(f"Total Shots: {total_shots}", variant="h6"),
                    spacing=2,
                )
            ),
    ...
    def render_box_plot(self, filtered_df: pd.DataFrame):
        ...
        data = [
            {
                "group": gear,
                "values": filtered_df[filtered_df["gear"] == gear][
                    "group_size"
                ].tolist(),
            }
        ...
        with elements("dashboard"):
            ...
            with dashboard.Grid(self.layout):
                ...
                with mui.Paper(
                    ...
                    logger.debug(
                        f"Rendered line chart for user {st.session_state.user_id}"
                    )
                # Box plot
                with mui.Paper(
                    key="box_plot",
                ...
                with mui.Paper(
                    ...
                    formatted_df["created_at"] = formatted_df["created_at"].dt.strftime(
                        "%Y-%m-%d %H:%M:%S"
                    )

                    mui.DataGrid(
                        columns=[
                            {"field": col, "headerName": col.title()}
