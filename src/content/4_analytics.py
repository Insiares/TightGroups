...
def filtering_data(df, ammo_filter, gear_filter, position_filter, start_date, end_date):
    logger.debug(
        f"Filtering data with {ammo_filter}, {gear_filter}, {position_filter}, {start_date}, {end_date}"
    )
    start_date = pd.to_datetime(start_date)
    end_date = pd.to_datetime(end_date)
    df_copied = df.copy()
    filt_df = df_copied[
        (df_copied["ammo"].isin(ammo_filter))
        & (df_copied["gear"].isin(gear_filter))
        & (df_copied["position"].isin(position_filter))
        & (df_copied["created_at"].between(start_date, end_date))
    ]
...
def create_boxplots(df: pd.DataFrame):
    box_plot = (
        ...
        .properties(title="Boxplot of Group Size by Setup", width=800, height=400)
    )
...
if len(df) == 0:
    ...
    filtered_df = filtering_data(
        df, ammo_filter, gear_filter, position_filter, start_date, end_date
    )
    ...
    chart = (
        alt.Chart(
            filtered_df.groupby(filtered_df["date"])
            .agg({"group_size": "mean"})
            .reset_index()
        )
        .mark_line(point=True)
        .encode(x="date:T", y="group_size:Q", tooltip=["date", "group_size"])
        .properties(width=800, height=400, title="Group Size Over Time")
    )

    with metrics_container:
        st.metric(
            "Mean Group Size",
            value=f"{mean_group_size:.2f}",
            delta=f"{mean_group_size - df['group_size'].mean():.2f}",
            delta_color="inverse",
            border=True,
        )
...
if st.button("Add a custom plot") or st.session_state.plot:
    ...
    split_column = st.selectbox(
        "Select a column to split data (optional):", options=["None"] + list(df.columns)
    )
    ...
    base = (
        alt.Chart(df)
        .mark_point()
        .encode(x=alt.X(x_axis, title=x_axis), y=alt.Y(y_axis, title=y_axis))
    )
